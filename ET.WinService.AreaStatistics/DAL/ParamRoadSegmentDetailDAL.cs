...
{
    public  class ParamRoadSegmentDetailDAL
    {
        public List<ParamRoadSegmentDetail> GetList(int RoadID)
        {
            try
            {
        ...
        /// <param name="db"></param>
        /// <returns></returns>
        public List<ParamRoadSegmentDetail> GetList(int RoadID, DbTransaction tran, Database db)
        {
            try
            {
                DataSet ds = null;
                ...
                try
                {
                    ...
                    string selectSql = @"select ID,RoadID,Longitude,Latitude from dbo.ANA_ParamRoadSegmentDetail where RoadID=@RoadID ";
                    ...
                    return DataUtil.ToList<ParamRoadSegmentDetail>(ds.Tables[0]);
                }
                catch (Exception ex)
                {
                    throw ex;
