...
namespace ET.WinService.ValidSpeedStatistics.DAL
{
    public class StudyRecordDAL
    {
        ...
        /// <summary>
        /// 写入学习记录的有效速度标识
        /// <para>速度统计标识：HasCalcSpeed设为1</para>
        /// <para>通过检查：ValidSpeed设为1</para>
        /// <para>不通过检查：ValidSpeed设为0</para>
        /// </summary>
        /// <param name="studyRecordID">学时ID</param>
        /// <param name="validSpeed">是否通过检查</param>
        public int Update_ValidSpeed_HasCalcSpeed(long studyRecordID, bool validSpeed)
        {
            try
            {
                Database db = DatabaseFactory.CreateDatabase(ConnectionControl.DefaultConnectionstring);
                ...
                try
                {
                    ...
                    string selectSql = @"UPDATE [w_StudyRecord_Result_Addition] 
                                         SET ValidSpeed = @ValidSpeed AND HasCalcSpeed = 1
                                         WHERE StudyRecordID = @StudyRecordID";
                    dbCommandWrapper.CommandText = selectSql;
                    db.AddInParameter(dbCommandWrapper, "@ValidSpeed", DbType.Boolean, validSpeed);
